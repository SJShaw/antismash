...
SHORT_DESCRIPTION = """
    Categorization of halogenases based on family and function.
...
GENERAL_FDH_PROFILES = [
    HmmSignature("all_conventional_FDH",
                 "Member of the Flavin-dependent halogenase family",
                 100, get_full_path(str(Path(__file__).parents[0]), "data", "all_conventional_FDH.hmm")),
    HmmSignature("unconventional_FDH",
                 "Unconventional flavin-dependent halogenase",
                 100, get_full_path(str(Path(__file__).parents[0]), "data", "unconventional_FDH.hmm")),
]
