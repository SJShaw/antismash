...
""" A feature to represent a RuleVis domain match """
...
T = TypeVar("T", bound="RuleVisDomain")

TOOL = "rulevis"


class RuleVisDomain(AntismashDomain):
    """ A feature representing a rule hit domain domain match.
    """
    ...
    def __init__(self, location: Location, description: str, protein_location: Location,
        ...
        """ Arguments:
                ...
                description: a string with a description from the rule hit domain database
                protein_location: the location within the parent CDS translation
                identifier: the rule hit domain identifier
        ...
        if not isinstance(description, str):
            raise TypeError("RuleVisDomain description must be a string, not %s" % type(description))
        self.description = description

        if not identifier:
            raise ValueError("RuleVisDomain identifier cannot be empty")
...
register_asdomain_variant(TOOL, RuleVisDomain)
