...
class ConditionMet:
    ...
    def __init__(self, met: bool, matches: Union[set[str], "ConditionMet"] = None,
        ...
        if isinstance(matches, ConditionMet):
            if presence_and_negation:
                raise ValueError("can't specify both matches and presence and negation")
            self.presence_and_negation = matches.presence_and_negation.copy()
        print(f"  ConditionMet: {self.matches=}, {self.ancillary_hits=}, presence={self.presence_and_negation}")
        if "b" in self.matches:
            raise
        if "b" in self.ancillary_hits.get("test_rule", {}):
            raise
...
#            return set()  # but not for nesting like "A or not (X and not B)"
        return {key: val for key, val in self.presence_and_negation.items()}
        if self.negated:
    ...
    def merge(self, other: Self, negate: bool = False) -> Self:
